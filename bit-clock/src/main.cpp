 ...
 *
 * 表示レイアウト (列 = 5個、各列5ビット、下段がLSB / 上段がMSB)
 *   1列目(左端) : 時   (0-23)  … 5ビットすべて使用
 *   2列目       : 分の十の位 (0-5)
 *   3列目       : 分の一の位 (0-9)
 *   4列目       : 秒の十の位 (0-5)
 *   5列目       : 秒の一の位 (0-9)
...
// 1列分をビット表示する
// bit0(LSB)を最下段に、上に行くほど上位ビットになるように表示
void showColumn(uint8_t col, uint8_t value, CRGB color) {
  for (uint8_t bit = 0; bit < MATRIX_HEIGHT; bit++) {
    uint8_t physRow = (MATRIX_HEIGHT - 1) - bit; // bit0 -> 最下段
...
void loop() {
  ...
  showColumn(0, hour,    COLOR_HOUR);
  showColumn(1, minTens, COLOR_MIN_TENS);
  showColumn(2, minOnes, COLOR_MIN_ONES);
  showColumn(3, secTens, COLOR_SEC_TENS);
  showColumn(4, secOnes, COLOR_SEC_ONES);
