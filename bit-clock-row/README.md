# 5x5 シリアルLED ビット時計 (MH-ET LIVE / ESP32) — 行バージョン

このバージョンは、列バージョン(main.cpp)から**行と列を入れ替えた**レイアウトです。
1行=1つの値(時/分/秒)を5ビットで表示し、**右端がビット0(LSB)**、左に行くほど
上位ビットになります。 - 行バージョン

このバージョンは、列バージョンから**行と列を入れ替えた**レイアウトです。
...
## 表示ルール
5行×5列のLEDを、行ごとに5ビットの2進数として使います。
各行の**右端がビット0(LSB)**、左に行くほど上位ビットです。
